...
Besides, all other predictors in the spatial error model, including `PCTVACANT`, `PCTSINGLES`, `PCTBACHMOR`, and `LNNBELPOV`, are also statistically significant.

- `PCTVACANT` has a coefficient of -0.0057 and is highly significant since the p-value is **less than 0.001**. This indicates that a higher vacancy rate is associated with lower median house values.
- `PCTSINGLES` has a coefficient of 0.0027, which is also statistically significant with a p-value of **less than 0.001**. This suggests that a higher percentage of single-unit households is positive associated with higher median house values.
- `PCTBACHMOR` has a coefficient of 0.0098 and is statistically significant with a p-value of **less than 0.001**, indicating that a higher percentage of residents with a bachelor's degree or higher is associated with higher median house values.
- `LNNBELPOV` has a coefficient of -0.0345 and is statistically significant with a p-value of **less than 0.001**, suggesting that a higher percentage of residents below the poverty line is negative associated with lower median house values.

Similar to spatial lag regression, when comparing these results with the OLS regression model, the coefficients for our predictors are smaller than in the OLS model. For example, the coefficient of `PCTVACANT` is -0.0192 in the OLS model, while it is -0.0057 in the spatial error model. This suggests that the OLS model overestimates the influence of these predictors due to omitted spatial autocorrelation.
...
Geographically Weighted Regression (GWR) is a spatial statistical method that allows regression coefficients to vary across space. This flexibility enables the model to better account for spatial heterogeneity, offering localized insights into relationships between predictors and the dependent variable (in this case, LNMEDHVAL).

To begin the analysis, we identify the optimal adaptive bandwidth for the GWR model using cross-validation.

```{r, message=FALSE, warning=FALSE}
# Prepare coordinates from sf geometry
...
Using the selected bandwidth, we estimate the GWR model.
...
We begin by comparing the R-squared of the GWR model with that of the OLS model.
...
The GWR model has a higher R-squared value than the OLS model, indicating that it explains more variance in the dependent variable.

Next, we compare model fit using Akaike Information Criteria (AIC and AICc). **AIC** (Akaike Information Criterion) measures the relative quality of a statistical model, balancing goodness of fit and complexity. **AICc** is a corrected version of AIC, more accurate for smaller sample sizes or models with many parameters—such as GWR. While AIC is commonly reported for OLS, Spatial Lag, and Spatial Error models, GWR reports AICc instead.

```{r}
# Extract AICc
aicc_gwr <- model_gwr$results$AICc

model_names <- c("OLS", "Spatial Lag", "Spatial Error", "GWR")
aic_vals <- c(aic_ols, aic_sl, aic_se, NA)
aicc_vals <- c(NA, NA, NA, aicc_gwr)

aic_table <- data.frame(
  Model = model_names,
  AIC = c(format(aic_ols, digits = 3), format(aic_sl, digits = 3), format(aic_se, digits = 3), "-"),
  AICc = c("-", "-", "-", format(aicc_gwr, digits = 3))
)

aic_table %>%
  kable(row.names = FALSE, caption = "Comparison of AIC and AICc Values Across Models") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
```

As shown above, the OLS, Spatial Lag, and Spatial Error models report AIC, while the GWR model reports AICc. Although the two metrics are not strictly comparable, they serve similar purposes. The lower AICc of the GWR model supports its superior fit in handling spatial heterogeneity.
...
```{r}
# Moran's I test for GWR residuals
residuals_gwr <- results_gwr$gwr.e
moran_mc_gwr <- moran.mc(residuals_gwr, queenlist, nsim = 999)
```

The Moran’s I for GWR residuals is approximately 0.06 with a p-value of 0.001, indicating a significant reduction in spatial autocorrelation compared to the OLS residuals (Moran’s I ≈ 0.31). This value is also lower than the Moran’s I values for the Spatial Lag and Spatial Error residuals, suggesting GWR is more effective at accounting for spatial dependence.

```{r}
ggplot(data.frame(res = residuals_gwr), aes(x = res)) +
  geom_histogram(bins = 50, fill = "#283d3b") +
  geom_vline(xintercept = moran_mc_gwr$statistic, color = "#c44536", linetype = 'dashed', size = 1) +
...
The histogram and scatterplot demonstrate that the GWR model significantly reduces spatial autocorrelation in the residuals compared to other models. This improvement in residual independence confirms the advantage of using a localized regression approach in spatial data contexts.
...
We begin by calculating the t-values for each coefficient:
...
make_tval_map <- function(df, variable, title) {
  breaks <- classInt::classIntervals(df[[variable]], n = 5, style = "quantile")$brks
  labels <- sapply(1:(length(breaks) - 1), function(i) {
    paste0(round(breaks[i], 2), " to ", round(breaks[i + 1], 2))
  })
  df$cat <- cut(df[[variable]], breaks = breaks, include.lowest = TRUE, labels = labels)

  ggplot(df) +
    geom_sf(aes(fill = cat), color = NA) +
    scale_fill_brewer(palette = "RdBu", name = "t-value") +
    labs(title = title) +
    theme_light() +
    theme(
...
p1 <- make_tval_map(data, "t_PCTVACANT", "t-value: PCTVACANT")
p2 <- make_tval_map(data, "t_PCTSINGLES", "t-value: PCTSINGLES")
p3 <- make_tval_map(data, "t_PCTBACHMOR", "t-value: PCTBACHMOR")
p4 <- make_tval_map(data, "t_LNNBELPOV", "t-value: LNNBELPOV")

(p1 + p2) / (p3 + p4)
```

These t-value maps reveal substantial geographic variation in predictor significance. For example, PCTVACANT is statistically significant in parts of West and North Philadelphia, while PCTBACHMOR shows significance more broadly across affluent regions. LNNBELPOV is notably significant in many central areas, suggesting poverty’s localized influence on house values.

Next, we explore model fit at the local level by visualizing the local R² values from the GWR model.
...
ggplot(data) +
  ...
  scale_fill_brewer(palette = "Purples", name = "Local R²") +
...
In this paper, we analyzed the spatial variation in median house values across Philadelphia using four regression techniques: Ordinary Least Squares (OLS), Spatial Lag, Spatial Error, and Geographically Weighted Regression (GWR). While OLS provided a useful baseline, we found significant spatial autocorrelation in its residuals (Moran’s I ≈ 0.31), indicating a violation of model assumptions. Both the Spatial Lag and Spatial Error models improved upon OLS by accounting for spatial dependencies, and the Likelihood Ratio tests confirmed that these models were statistically superior to OLS. However, GWR outperformed all other models, achieving the highest R² (≈ 0.72) and the lowest residual spatial autocorrelation (Moran’s I ≈ 0.06). Moreover, it provided localized insight into how each predictor’s influence varied across neighborhoods, revealing nuanced spatial patterns that global models could not detect. Therefore, GWR was the best model overall for explaining the spatial variation in housing values in Philadelphia.

Despite its strengths, the analysis faced several limitations. First, key OLS assumptions were violated. The Breusch-Pagan, Koenker-Bassett, and White tests all indicated significant heteroscedasticity, meaning that the variance of residuals was not constant. Additionally, the Jarque-Bera test showed non-normal residuals, and significant spatial autocorrelation was present in the OLS residuals, violating the independence assumption. While spatial models addressed the issue of spatial autocorrelation, they still rely on assumptions such as linearity and homoscedasticity, which may not hold perfectly in real-world spatial datasets. Lastly, multicollinearity among predictors was not directly tested and could affect coefficient stability, particularly in GWR.

It’s important to clarify a common point of confusion: weighted residuals and spatial lag model residuals are not the same. Weighted (or spatially lagged) residuals are created by averaging the residuals of a spatial unit's neighbors, as defined by a spatial weights matrix. These are used in diagnostic plots (e.g., Moran’s I scatterplots) to detect spatial patterns in residuals. In contrast, spatial lag model residuals are the actual model errors (observed - predicted) from a regression that includes a spatially lagged dependent variable as a predictor. These residuals result after accounting for the endogenous influence of nearby outcome values. It is critical to use precise terminology: “spatially lagged residuals” should not be used to describe residuals from a spatial lag model; rather, it refers to the spatial average of residuals from another model (e.g., OLS or GWR).
