...
### Global Moran'I
First, we want to see if our dependent vairable, `LNMEDHVAL` (In-transformed median housing value),exhibits spatial autocorrelation. We first apply queen contiguity weighting to define spatial `LNMEDHVAL`. The result value is **0.794**, which is close to +1, indicating a relatively strong positive spatial autocorrelation. 
...
```
Then, we use `moran.mc` to test statistical significance. The **p-value** is less than **0.005**, confirming that the observed spatial autocorrelation is statistically significant. 
...
globalmoranMC
```
To further illustrate this result, we generate a histogram of Moran’s I values based on **random permutations**. The value of Moran’s I in our data set (0.794) is  the much higher than the values shown in the histogram of Moran’s I values generated from random permutations, indicating our dependent variable `LNMEDHVAL` is not randomly distributed. It reinforces the presence of strong spatial correlation in our dependent variable `LNMEDHVAL`.

```{r global moran histogram plot, message=FALSE, warning=FALSE}

moranMCres<-globalmoranMC$res
...
To visualize the spatial relationship further, we also create a plot. If no spatial autocorrelation were present, the plot would display no clear pattern. However, our plot shows distinct clusters, supporting the evidence of spatial autocorrelation in `LNMEDHVAL`. 
...
To viluaize the results, we created a **significance map** to show the P-value of Local Morans'I. The p-value tells us whether the clustering is statistically significant (real) or not. In the map, **dark blue (p ≤ 0.001), blue (p ≤ 0.01), and light blue (p ≤ 0.05) indicate areas where similar median housing values are grouped together**, showing a significant spatial pattern. In contrast, **white (p > 0.05) represents areas without a strong pattern**, where median housing values are more randomly distributed. In Philadelphia, the map reveals notable clustering in areas including **Center City, City Line Avenue,Roxborough and Germantown, Far Northeast Philadelphia**, indicating statistically significant spatial correlations in these regions. 
...
We also created a **Moran scatter plot** to figure out the relationship between each unit’s median housing value and its local Moran’s I. The plot helps identify spatial autocorrelation, showing whether areas with high or low values are clustered together or isolated. 
...
To visualize it more clearly, we generated a  **Cluster Map** to identify different types of clustering patterns. This map helps us analyze how housing values are distributed and whether they form significant spatial clusters.

- Center City, Roxborough, Germantown, and Far Northeast Philadelphia showed **High-High (HH)** clusters, meaning areas with high median housing values are surrounded by other high-value areas. These regions typically indicate **concentrated economic activity and rich neighborhoods**.

- City Line Avenue, the northern part of North Philadelphia, the northeastern part of University City, and the northwestern part of South Philadelphia exhibited **Low-Low (LL)** clusters, where areas with low median housing values are surrounded by other low-value areas. These clusters may indicate **economically underdeveloped or lower-income neighborhoods**.

- Three census blocks in South Philadelphia showed **High-Low (HL)** clusters, where high-value areas are surrounded by low-value areas. These locations represent spatial outliers, potentially indicating **newly developed commercial or residential projects in historically low-income neighborhoods** or **gentrification neighborhood**.

- Three scattered census blocks in Philadelphia exhibited **Low-High (LH)** clusters, meaning areas with low median housing values are surrounded by high-value areas. These clusters may correspond to **historically preserved districts with strict development regulations** or **industrial or undeveloped land within high-value urban centers**.
