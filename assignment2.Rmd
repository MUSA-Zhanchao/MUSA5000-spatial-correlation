...
- **Null Hypothesis (\(H_0\))**: No local spatial autocorrelation at location \(i\) (\(I_i \approx 0\)). The values of the variable at location \(i\) have no significant relationship with the values of the variable at neighboring locations \(j\).
  -  \(I_i\) represents Moran’s I at location \(i\).

- **Alternative Hypothesis (\(H_a\))**: Presence of local spatial autocorrelation at location \(i\) (\(I_i \neq 0\)). The values at location \(i\) are either very similar to those at neighboring locations (**positive spatial autocorrelation**) or significantly different from nearby values (**negative spatial autocorrelation**).
