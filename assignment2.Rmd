...
Our previous OLS regression analysis found that the percentage of vacant houses, single-family homes, and residents with at least a bachelor’s degree significantly impact median house values of the census block group, while poverty levels also show a negative association. The model explains approximately 66% of the variance in log-transformed median house values, indicating that the selected predictors play a significant role in housing price patterns.

Desptie the assumptions we detected in OLS regression, we will dectect other two assuptions, **Homoscedasticity** and **Normality of Errors**. 
...
However, the histogram of residuals from the previous OLS regression suggests a generally normal distribution with a bell-shaped curve centered around zero. **This visual check indicates that the residuals are approximately normally distributed in terms of their values, but they may not be normally distributed in a spatial context.** To address this potential issue, alternative methods, such as incorporating spatial weights, may be necessary to account for spatial dependencies and improve the model's accuracy.
...
Then, we regress the OLS standardized residuals `OLS_RESIDU` on the spatial lag `WT_RESIDU`. The **beta coefficient** of the lagged residuals is 0.732, indicating a positive spatial relationship, consistent with the slope observed in the scatter plot. The **p-value **is less than 0.0001, confirming that this relationship is statistically significant. This confirms the presence of significant spatial autocorrelation in the residuals, meaning that nearby residuals are related and not independent.

```{r}
# scatterplot of OLS_RESIDU by WT_RESIDU
ggplot(data, aes(x = WT_RESIDU, y = OLS_RESIDU)) +
  geom_point(color = "#283d3b", alpha = 0.6) +
  geom_smooth(method = "lm", se = FALSE, color = "#c44536") +
...
By regressing OLS residuals on their queen neighbors and analysis the Global Morans'I, we find **significant spatial autocorrelation in the residuals**, indicating that errors are not randomly distributed across space. This suggests that the OLS model violates the assumption of **Normality of Residuals**, which can lead to biased standard errors, inefficient estimates, and misleading statistical inferences. In particular, hypothesis tests and confidence intervals may no longer be valid, potentially affecting policy decisions or further analyses based on the model.
